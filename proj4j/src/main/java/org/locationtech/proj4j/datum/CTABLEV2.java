...
public final class CTABLEV2 {
    ...
    public static boolean testHeader(byte[] header) {
        try {
            return containsAt("CTABLE V2".getBytes(CharEncoding.US_ASCII),
                    header, 0);
    ...
    public static Grid.ConversionTable init(DataInputStream definition) throws IOException {
        ...
        if (!containsAt("CTABLE V2".getBytes(CharEncoding.US_ASCII), header, 0))
        ...
        table.id = new String(id_bytes, 0, nullPosition, CharEncoding.US_ASCII)
                .trim();
        table.ll = ll;
        table.del = del;
        table.lim = lim;
