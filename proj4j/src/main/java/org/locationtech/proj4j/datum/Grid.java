...
public final class Grid implements Serializable {
    ...
    NTv2GridShiftFile ntv2GridShift = null;
    ...
    public static void shift(List<Grid> grids, boolean inverse, ProjCoordinate in) {
        ...
        for (Grid grid : grids) {
            ...
            if (grid.ntv2GridShift != null) {
                NTv2GridShift gs = new NTv2GridShift();
                gs.setLatDegrees(Math.toDegrees(in.y));
                gs.setLonPositiveEastDegrees(Math.toDegrees(in.x));
                try {
                    if (inverse) {
                        grid.ntv2GridShift.gridShiftReverse(gs);
                    } else {
                        grid.ntv2GridShift.gridShiftForward(gs);
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
                output.lam = ProjectionMath.degToRad(gs
                        .getShiftedLonPositiveEastDegrees());
                output.phi = ProjectionMath.degToRad(gs.getShiftedLatDegrees());
            } else {
                ConversionTable table = grid.table;
                if (table != null) {
                    double epsilon = (Math.abs(table.del.phi) + Math
                            .abs(table.del.lam)) / 10000d;
                    // Skip tables that don't match our point at all
                    if (table.ll.phi - epsilon > input.phi
                            || table.ll.lam - epsilon > input.lam
                            || (table.ll.phi + (table.lim.phi - 1)
                                    * table.del.phi + epsilon < input.phi)
                            || (table.ll.lam + (table.lim.lam - 1)
                                    * table.del.lam + epsilon < input.lam))
                        continue;
                ...
                while (grid.child != null) {
                    ...
                    for (child = grid.child; child != null; child = child.next) {
                        ...
                        double epsilon0 = (Math.abs(t.del.phi) + Math
                                .abs(t.del.lam)) / 10000d;

                        if (t.ll.phi - epsilon0 > input.phi
                                || t.ll.lam - epsilon0 > input.lam
                                || (t.ll.phi + (t.lim.phi - 1) * t.del.phi
                                        + epsilon0 < input.phi)
                                || (t.ll.lam + (t.lim.lam - 1) * t.del.lam
                                        + epsilon0 < input.lam))
                            continue;
    ...
    private static PolarCoordinate nad_intr(PolarCoordinate t, ConversionTable table) {
        ...
        index = indx.phi * (table.lim.lam) + indx.lam;
        f00 = table.cvs[index++];
        f10 = table.cvs[index];
        index += table.lim.lam;
        f11 = table.cvs[index--];
    ...
    private static Grid gridinfoInit(String gridName) throws IOException {
        ...
        grid.format = "missing";
        grid.gridOffset = 0;
        DataInputStream gridDefinition = null;
        try {
            gridDefinition = resolveGridDefinition(gridName);
            ...
            if (CTABLEV2.testHeader(header)) {
                ...
                grid.ntv2GridShift = NTV2.loadNTv2(gridDefinition);
            }
        } finally {
            if (gridDefinition != null) {
                gridDefinition.close();
            }
        }
        return grid;
    }
    ...
    public boolean equals(Object that) {
        if (that instanceof Grid) {
            ...
            if (gridName != null && !gridName.equals(g.gridName))
                return false;
            if (fileName == null && g.fileName != null) return false;
            if (fileName != null && !fileName.equals(g.fileName))
                return false;
            if (format == null && g.format != null) return false;
            if (format != null && !format.equals(g.format))
                return false;
            if (table == null && g.table != null) return false;
            if (table != null && !table.equals(g.table))
                return false;
            if (next == null && g.next != null) return false;
            if (next != null && !next.equals(g.next))
                return false;
            if (child == null && g.child != null) return false;
            if (child != null && !child.equals(g.child))
                return false;
            return true;
        } else {
            return false;
