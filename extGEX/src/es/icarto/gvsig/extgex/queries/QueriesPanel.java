...
public class QueriesPanel extends ValidatableForm implements ActionListener {
    ...
    protected void initWidgets() {
        ...
        customQueriesB = (JButton) formPanel.getComponentByName(ID_CUSTOMQUERIES);
    ...
    private void executeValidations(boolean customized) throws SQLException {
        ...
        QueryFilters queryFilters = new QueryFilters(getSelectedKVForWidget(DBNames.FIELD_TRAMO_FINCAS),
                getSelectedKVForWidget("uc"), getSelectedKVForWidget(DBNames.FIELD_AYUNTAMIENTO_FINCAS),
                getSelectedKVForWidget(DBNames.FIELD_PARROQUIASUBTRAMO_FINCAS));
        ...
        if (customized) {
            ...
            if (queryCode.equals("custom-exp_finca")) {
                columns = Utils.getFields(resource.getPath(), DBNames.SCHEMA_DATA, FormExpropiations.TABLENAME);
                for (Field f : columns) {
                ...
                String[] tableColumns = DBSession.getCurrentSession().getColumns(DBNames.SCHEMA_DATA,
                        FormExpropiations.TABLENAME);
            ...
            query = buildQuery(query, customiceDialog.getFields(), customiceDialog.getOrderBy());
        }
    ...
    private String[] doQuery(String queryCode, QueryFilters filters) throws SQLException {
        ...
        // query
        contents[0] = query.replaceAll("\\[\\[WHERE\\]\\]", filters.getWhereClauseByLocationWidgets(hasWhere));
        // description
    ...
    private String buildQuery(String query, List<Field> fields, List<Field> orderBy) {

        String subquery = query;

        if (fields.size() > 0) {
            subquery = buildFields(fields, "SELECT   ") + query.substring(query.indexOf(" FROM"), query.length());
        }
    ...
    private List<Field> parseQuery(String query, String[] columns) {
        ...
        String fieldsStr = query.substring(query.indexOf("SELECT ") + 7, query.indexOf(" FROM"));
