...
public class CADToolAdapter extends Behavior {
	...
	/*
	 * super.paintComponent(g); if (CADExtension.getCADToolAdapter()!=this)
	 * return;
	 * 
	 * if (adjustedPoint != null) { Point2D p = null; if (mapAdjustedPoint
	 * != null) { p = mapAdjustedPoint; } else { p =
	 * getMapControl().getViewPort().toMapPoint(adjustedPoint); } if
	 * (!cadToolStack.isEmpty()) ((CADTool) cadToolStack.peek())
	 * .drawOperation(g, p.getX(), p.getY()); } drawCursor(g);
	 * getGrid().drawGrid(g);
	 */
	...
	/*
	 * if (e.getButton() == MouseEvent.BUTTON1) { ViewPort vp =
	 * getMapControl().getMapContext().getViewPort(); Point2D p;
	 * 
	 * if (mapAdjustedPoint != null) { p = mapAdjustedPoint; } else { p =
	 * vp.toMapPoint(adjustedPoint); } transition(new double[] { p.getX(),
	 * p.getY() }, e, ABSOLUTE); }
	 */
	if (e.getButton() == MouseEvent.BUTTON1) {
	    boolean followActivated = CADStatus.getCADStatus()
		    .isFollowGeometryActivated();
	    System.out.println("multi: " + getCadTool().isMultiTransition()
		    + " follow: " + followActivated);
	    if (otherMapAdjustedPoints == null
		    || otherMapAdjustedPoints.size() == 0
		    || !getCadTool().isMultiTransition() || !followActivated) {
	...
	if (cadStatus.isVertexActivated() || cadStatus.isNearLineActivated()
		...
		if (lyrVect.isVisible()) {
		    // La lista de snappers está siempre ordenada por prioridad.
		    // Los de mayor
			...
			for (int i = 0; i < snappers.size(); i++) {
				...
				// }
				// snapVisitor = new
				// SnappingVisitor((ISnapperVectorial)
				...
				double distAux = theSnappedPoint
					.distance(point);
				if (minDist > distAux) {
				    minDist = distAux;
				    usedSnap = theSnapper;
	...
	/*
	 * if (usedSnap != null) return minDist; return Double.MAX_VALUE;
	 */
	...
	if (PluginServices.getMDIManager().getActiveWindow() instanceof View) {
	    ...
	    if (question != null
		    && (lastQuestion == null
		    || !(lastQuestion.equals(question)) || questionAsked)) {
	...
	if (actionCommand.equals("eliminar")) {
		...
		// if (((InsertionCADTool) ct).getFormState() ==
		// InsertionCADTool.FORM_ACCEPTED) {
		// ct.transition(PluginServices.getText(this,
		// "accept_form"));
		// askQuestion();
		// } else if (((InsertionCADTool) ct).getFormState() ==
		// InsertionCADTool.FORM_CANCELLED) {
		// ct.transition(PluginServices.getText(this,
		// "cancel_form"));
		// }
		// }
    ...
    // [LBD comment] con esto limpio el ultimo punto pulsado para reinicializar
    // el seguimiento de
    // los snappers
