...
public class SIGAFormatter extends Format {
    ...
    public static String formatPkForDisplay(Number pk) {
        if ((pk == null)) {
            return "";
        }
        try {
            return String.format("%3.3f", pk).replace(".", ",");
    ...
    public static String formatValue(Object o) {
        if (o == null) {
            ...
            return doubleFormatOnDisplay.format(o);
    ...
    public static String writeDBValueFormatted(ResultSet rs, int column) {
        ...
        try {
            ...
            } else if (rs.getMetaData().getColumnName(column).equalsIgnoreCase("fecha_puesta_servicio")) {
                valueFormatted = rs.getString(column);
            } else if (rs.getMetaData().getColumnType(column) == 4 || rs.getMetaData().getColumnType(column) == 8
