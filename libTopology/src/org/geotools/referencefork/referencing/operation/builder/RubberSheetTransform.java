 ...
 * {@linkplain org.geotools.referencefork.referencing.operation.builder.RubberSheetBuilder
 ...
 *
 * @since 2.4
 * @source $URL: http://svn.geotools.org/geotools/trunk/gt/modules/library/referencing/src/main/java/org/geotools/referencing/operation/builder/RubberSheetTransform.java $
 * @version $Id: RubberSheetTransform.java 28969 2008-01-27 18:30:46Z acuster $
 * @author Jan Jezek
 *
 * @todo Consider moving this class to the {@linkplain
 *       org.geotools.referencefork.referencing.operation.transform} package.
 */
class RubberSheetTransform extends AbstractMathTransform implements MathTransform2D {
     ...
     *        {@linkplain org.geotools.referencefork.referencing.operation.builder.algorithm.TINTriangle}
    ...
    public void transform(double[] srcPts, int srcOff, final double[] dstPt, int dstOff, int numPts)
        ...
        for (int i = srcOff; i < numPts; i++) {
            ...
            try{
            	...
            	//there isnt any triangle for this point, so we dont transform the point
            	dstPt[2 * i] = pos.getX();
            	dstPt[(2 * i ) + 1] = pos.getY();
            }
        }//for
    }
