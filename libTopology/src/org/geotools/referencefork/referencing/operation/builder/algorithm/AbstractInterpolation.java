 ...
 */
package org.geotools.referencefork.referencing.operation.builder.algorithm;
...
//            	org.opengis.geometry.DirectPosition dp = ref.create(dpOld.getCoordinates(), dpOld.getCoordinateReferenceSystem());
    ...
    private void buildCoverage() {
        ...
        for (int i = 0; i <= yNumCells; i++) {
            for (int j = 0; j <= xNumCells; j++) {
//            	org.opengis.geometry.DirectPosition oldDp = new DirectPosition2D(env.getLowerCorner().getOrdinate(0)+ (j * dx), 
//            											env.getLowerCorner().getOrdinate(1) + (i * dy));
//            	org.opengis.geometry.DirectPosition dp = ReferencingUtil.getInstance().create(oldDp.getCoordinates(),oldDp.getCoordinateReferenceSystem());
//            	gridValues[(i * (1 + xNumCells)) + j] = getValue(dp);
