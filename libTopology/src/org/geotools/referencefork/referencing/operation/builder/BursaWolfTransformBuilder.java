...
import org.geotools.referencefork.referencing.operation.transform.GeocentricTranslation;
 ...
 * org.geotools.referencefork.referencing.operation.builder.MappedPosition MappedPosition}.
 ...
 *
 * @since 2.4
 * @source $URL: http://svn.geotools.org/geotools/trunk/gt/modules/library/referencing/src/main/java/org/geotools/referencing/operation/builder/BursaWolfTransformBuilder.java $
 * @version $Id: BursaWolfTransformBuilder.java 29769 2008-04-02 17:10:40Z desruisseaux $
 * @author Jan Jezek
 *
 */
public class BursaWolfTransformBuilder extends MathTransformBuilder {
     ...
     */
    //TODO VER PORQUE DEBE RETORNAR 3, CUANDO EL RESTO DE TRANSFORMACIONES SON 2D
    //DE MOMENTO COMENTADO PARA EVITAR ERRORES EN TIEMPO DE EJECUCION AL CHEQUEAR LAS DIMENSIONES
//    @Override
...
//    }
    ...
    protected GeneralMatrix getx() {
        ...
        for (int j = 0; j < (x.getNumRow()); j = j + 3) {
            ...
            if(sourcePoints[j / 3].getCoordinates().length > 2)
            	x.setElement(j + 2, 0, sourcePoints[j / 3].getCoordinates()[2]);
            else 
            	x.setElement(j + 2, 0, 0);
        }
    ...
    protected GeneralMatrix getX() {
        ...
        for (int j = 0; j < (X.getNumRow()); j = j + 3) {
            ...
            if(targetPoints[j / 3].getCoordinates().length > 2)
