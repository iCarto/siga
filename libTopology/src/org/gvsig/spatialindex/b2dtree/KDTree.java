...
public class KDTree {
    ...
    */
    public Object searchNearest(double [] key) throws KeySizeException {
	
	Object [] nbrs = nearest(key, 1);
	return nbrs[0];
    }
    
    public Object searchNearest(double x, double y) throws KeySizeException {
    	double[] key = {x,y};
    	Object [] nbrs = nearest(key, 1);
    	return nbrs[0];
     }
    ...
    */
    ...
    public LinkedList getLinkedList(){
       ...
       public Object preorederTestContain(PointDT P){
       	// System.out.println("AA");
       	 change = true;
       	 testContain(m_root, P);
       	 return first.v;
       	 
        }
       
       private void testContain(KDNode current, PointDT P){
         	 
      	 if (!change||current == null)
      		 return;
      	 if (!current.deleted){
      		 //System.out.println("AA");
      		 if (((TriangleDT)current.v).contains(P)){
      			 first = current;
      			 change = false;
          		 return;
          	 }
      	 }	 
      	 testContain(current.left,P);
      	 testContain(current.right,P);
      	 
       }
       
       public Object preorderTestContainTwoPoints(PointDT A, PointDT B){
         	 change = true;
      	 testContain(m_root, A,B);
      	 return first.v;
       }
       
       private void testContain(KDNode current, PointDT A, PointDT B){
      	 
      	 if (!change||current == null)
      		 return;
      	 if (!current.deleted){
      		 if (((TriangleDT)current.v).containsTwoPoints(A, B)){
      			 first = current;
      			 change = false;
          		 return;
          	 }
      	 }	 
      	 testContain(current.left, A, B);
      	 testContain(current.right, A, B);
