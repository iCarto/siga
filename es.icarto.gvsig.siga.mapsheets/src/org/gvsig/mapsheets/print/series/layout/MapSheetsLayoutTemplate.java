...
public class MapSheetsLayoutTemplate extends Layout implements IMapSheetsIdentified {
    ...
    static {
        ...
        try {
            IFFrame txtf = FrameFactory.createFrameFromName(FFrameTextFactory.registerName);
        ...
        try {
            IFFrame viewf = FrameFactory.createFrameFromName(FFrameViewFactory.registerName);
    ...
    public MapSheetsLayoutTemplate(MapSheetGrid gr, ProjectView pv, Layout size_dpi_layout) {
        ...
        // set attributes
        Attributes param_atts = size_dpi_layout.getLayoutContext().getAtributes();
        ...
        int res_ind = MapSheetsUtils.dpiToPrintQualityIndex(Attributes.DPI);
        atts.setResolution(res_ind);

        PrintQuality pq = MapSheetsUtils.dpiToPrintQuality(Attributes.DPI);
        ...
        size_dpi_layout.getLayoutContext();
        LayoutContext.nums.clear();
    }
    ...
    private void updateViewFrameSize() {
        ...
        try {
            ...
            MapSheetFrameView msfv = getMainViewFrame();
            msfv.setBoundBox(new Rectangle2D.Double(msfv.getBoundBox().getMinX(), msfv.getBoundBox().getMinY(), w_cm,
                    h_cm));
            this.repaint();
    ...
    public void init(ArrayList fd_list, ArrayList ind_list, ArrayList tem_list, double left_cm, double top_cm,
            boolean is_test, AudasaTemplate audasaTemplate) {
        ...
        if (pView.getMapOverViewContext() != null && pView.getMapOverViewContext().getViewPort() != null
        ...
        viewFrame.setBoundBox(new Rectangle2D.Double(left_cm, top_cm, w_cm, h_cm));
        viewFrame.setFrameLayoutFactory(viewFrameFactory);

        viewFrame.setLayout(this);
        viewFrame.setName("main_frame");
        viewFrame.setTypeScale(IFFrameUseFMap.MANUAL);
        NumericValue scale;
        try {
            scale = ((NumericValue) grid.getCommonAtts().get(MapSheetGrid.ATT_NAME_SCALE));
        ...
        for (int i = 0; i < n; i++) {
            ...
            tem_txt.setBoundBox(new Rectangle2D.Double(right_col_pos_cm, act_flds_top_cm + i
                    * (0.1 * av_height_cm + right_col_sep_cm), right_col_wit_cm, 0.1 * av_height_cm));
        ...
        for (int i = 0; i < framesFromTemplate.length; i++) {
            ...
            if (framesFromTemplate[i] instanceof FFrameText) {
                if (audasaTemplate.hasKey(((FFrameText) framesFromTemplate[i]).getTitle())) {
                    ((FFrameText) framesFromTemplate[i]).setText(audasaTemplate
                            .getProperty(((FFrameText) framesFromTemplate[i]).getTitle()));
                    // } else {
    ...
    public XMLEntity getXMLEntity() {
        ...
        resp.putProperty("dpi", Attributes.DPI);
    ...
    public void setXMLEntity(XMLEntity xml, Project p) throws OpenException {
        ...
        Attributes atts = getLayoutContext().getAtributes();
        Attributes.DPI = xml.getIntProperty("dpi");
    ...
    public void showPrintDialog(PrinterJob job, boolean show_setts) {
        if (job != null) {
            ...
            PrintRequestAttributeSet aux_att = getLayoutContext().getAtributes().toPrintAttributes();
            ...
            aux_m_cachePrintServices = PrintServiceLookup.lookupPrintServices(flavor, null);
            ...
            if (aux_m_cachePrintService == null && defaultService == null && aux_m_cachePrintServices.length > 0) {
                defaultService = aux_m_cachePrintServices[0];
            }

            if ((defaultService == null) && (aux_m_cachePrintService == null)) {
                JOptionPane.showMessageDialog((Component) PluginServices.getMainFrame(),
                        PluginServices.getText(this, "ninguna_impresora_configurada"));

                return;
            }

            try {
                aux_m_cachePrintService = ServiceUI.printDialog(null, 200, 200, aux_m_cachePrintServices,
                        defaultService, flavor, aux_att);
            ...
            if (aux_m_cachePrintService != null) {
                ...
                if (show_setts) {
                    ...
                    if (MapSheetsPrintExtension.PRINTER_SETTINGS_PRINTER_NAME == null
                        ...
                        String fname = System.getProperty("user.home") + File.separator + "pri-sett-"
                                + System.currentTimeMillis() + ".txt";

                        boolean saved = MapSheetsUtils.printerSettingsSaveRestore(printer_nam, fname, true);
                        if (saved) {
