...
public class PrintSelectionDialog extends JPanel implements IWindow, ActionListener, ListSelectionListener {
    ...
    private boolean workaroundAlreadyExecuted;
    ...
    private JList getSheetList() throws Exception {
        if (sheetList == null) {
            ...
            for (int i = 0; i < cont; i++) {
                SheetComboItem item = new SheetComboItem(gr.getGraphic(i));
                int position = 0;
                if (dlm.getSize() > 0) {
                    do {
                        if (item.toString().compareTo(dlm.get(position).toString()) < 0) {
                            break;
                        }
                        position++;
                    } while (position < dlm.getSize());
                }
                dlm.add(position, new SheetComboItem(gr.getGraphic(i)));
            }
            ...
            // int[] sheets = MapSheetsUtils.filterMapSheetGridSheets(dlm, gr, "MUNICIPIO",
            // ValueFactory.createValue("TEST"));
    ...
    private JButton getPrintPrinterButton(boolean less_space) {
        if (printPrinterButton == null) {
            ...
            printPrinterButton.setPreferredSize(new Dimension(
                    (int) (printPrinterButton.getPreferredSize().width * 2.5), (int) (printPrinterButton
    ...
    private JButton getCancelButton() {
        if (closeButton == null) {
            ...
            closeButton.setPreferredSize(new Dimension((int) (closeButton.getPreferredSize().width * 2.5),
    ...
    private JLabel getPdfFilesPrefixLabel() {
        if (pdfFilesPrefixLabel == null) {
            pdfFilesPrefixLabel = new JLabel(PluginServices.getText(null, "File_name_prefix"), new ImageIcon(
                    getClass().getClassLoader().getResource("images" + File.separator + "help.png")),
    ...
    private JPanel getOtherSettingsPanel() {
        ...
        if (otherSettingsPanel == null) {
            ...
            otherSettingsPanel.setBorder(BorderFactory.createTitledBorder(null,
                    PluginServices.getText(this, "Print_options"), TitledBorder.DEFAULT_JUSTIFICATION,
                    TitledBorder.DEFAULT_POSITION, new Font("Dialog", Font.BOLD, 12), new Color(51, 51, 51)));

            int sepDistance = LayoutStyle.getInstance().getPreferredGap(getPdfOutputDirLabel(),
                    getPdfOutputDirTextField(), LayoutStyle.ComponentPlacement.UNRELATED, SwingConstants.EAST, this);
            ...
            horizontal.addComponent(getBackLayerCombo(), GroupLayout.PREFERRED_SIZE, getBackLayerCombo()
                    .getPreferredSize().width, GroupLayout.PREFERRED_SIZE);
            vertical.addComponent(getBackLayerCombo(), GroupLayout.PREFERRED_SIZE, getBackLayerCombo()
                    .getPreferredSize().height, GroupLayout.PREFERRED_SIZE);
            ...
            pdfPrefixHorizontal.addGap(LayoutStyle.getInstance().getPreferredGap(getPdfFilesPrefixTextField(),
                    getPdfFilesPrefixTextField(), LayoutStyle.ComponentPlacement.INDENT, SwingConstants.EAST, this));
            ...
            pdfOutputDirFileChooser.setFileSelectionMode(javax.swing.JFileChooser.DIRECTORIES_ONLY);
            ...
            // We add gaps before and after the main groups and also a group which contains the leading one and the
            // centered one
            layout.setHorizontalGroup(layout
                    .createSequentialGroup()
                    .addGap(LayoutStyle.getInstance().getPreferredGap(getPdfOutputDirLabel(),
                            getPdfOutputDirTextField(), LayoutStyle.ComponentPlacement.INDENT, SwingConstants.EAST,
                            this))
                    .addGroup(
                            layout.createParallelGroup(GroupLayout.Alignment.CENTER).addGroup(horizontal)
                                    .addGroup(printSettings))
                    .addGap(LayoutStyle.getInstance().getPreferredGap(getPdfOutputDirLabel(),
                            getPdfOutputDirTextField(), LayoutStyle.ComponentPlacement.INDENT, SwingConstants.EAST,
                            this)));
    ...
    private JPanel getSheetListPanel() throws Exception {
        ...
        if (sheetListPanel == null) {
            ...
            sheetListPanel.setBorder(BorderFactory.createTitledBorder(null, PluginServices.getText(this, "Preview"),
                    TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, new Font("Dialog", Font.BOLD,
    ...
    private JScrollPane getSheetsScroll() throws Exception {
        ...
        if (sheetScroll == null) {
            ...
            sheetScroll.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
        }
        return sheetScroll;
    }
    ...
    public JCheckBox getUseThisBackLayerChk() {
        if (useThisBackLayerChk == null) {
            ...
            useThisBackLayerChk.setText(PluginServices.getText(this, "Use_back_layer"));
        ...
        if (src == getPrintPrinterButton(getIsWindows())) {
            ...
            if (printPdfCheckbox.isSelected()) {
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(this, PluginServices.getText(this, "Directory_not_writable"),
                            PluginServices.getText(this, "Error"), JOptionPane.ERROR_MESSAGE);
                    return;
                }
                ...
                PrintTaskWindow.startPrintTask(layout_template, this.getPrintAllRB().isSelected(),
                        highlightSelectionCheckbox.isSelected(), sel_back == null ? null : sel_back.getLayer(),
                        pdfOutputDirTextfield.getText(), pdfFilesPrefixTextfield.getText(), this, false);
                getBackLayerCombo().setEnabled(getUseThisBackLayerChk().isSelected());
                return;
                ...
                JOptionPane.showMessageDialog(layout_template,
                        PluginServices.getText(this, "Se_va_a_cerrar_mapa_para_acelerar_proceso"),
                        PluginServices.getText(this, "Print"), JOptionPane.INFORMATION_MESSAGE);
                ...
                MapSheetsUtils.printMapSheetsLayout(layout_template, sel_back == null ? null : sel_back.getLayer(),
    ...
    private void workaroundToFixFirstGrid(SheetComboItem sci) {
        // Ver #1828
        // Si se llama desde el constructor no se arregla nada
        if (workaroundAlreadyExecuted) {
            return;
        }
        workaroundAlreadyExecuted = true;

        if (sci == null) {
            if (sheetList.getModel().getSize() > 0) {
                sci = (SheetComboItem) sheetList.getModel().getElementAt(0);
            }
        }

        try {

            MapSheetGridGraphic o = sci.getObject();
            XMLEntity org_xml = o.getXMLEntity();
            MapSheetGridGraphic newFoo = new MapSheetGridGraphic();
            newFoo.setXMLEntity(org_xml);
            IGeometry a = newFoo.getGeom();
            com.vividsolutions.jts.geom.Geometry b = a.toJTSGeometry();
            b = b.buffer(100);
            IGeometry c = FConverter.jts_to_igeometry(b);
            newFoo.setGeom(c);
            layout_template.updateWithSheet(newFoo);
            // String code = sci.toString();
            // layout_template.updateAudasaSheetCode(code);

            // Para que se vea bien la vista inicial sin seleccionar nada en la pantalla
            // No funciona
            // layout_template.updateWithSheet(sci.getObject());
            // layout_template.updateAudasaSheetCode(code);
