...
public class MultiRasterDataset implements IRasterDataSource {
	 ...
	 */
	private void initBufferToNoData(IBuffer raster, BandList bandList) throws InterruptedException {
	 ...
	 * @throws InterruptedException 
	 */
	private void mergeBuffers(IBuffer b, IBuffer[] bDataset, BandList bandList) throws InterruptedException {
	...
	public IBuffer getWindowRaster(double ulx, double uly, double w, double h, boolean adjustToExtent) 
		...
		int height = (int)Math.abs(Math.ceil(p2.getY()) - Math.floor(p1.getY()));
		...
		if(isReadOnly()) {
			...
			if(rb instanceof RasterReadOnlyBuffer) {
				try {
					((RasterReadOnlyBuffer)rb).setBufferParams(this, (int)Math.floor(p1.getX()), (int)Math.floor(p1.getY()), (int)Math.ceil(p2.getX()), (int)Math.ceil(p2.getY()), bandList);
				} catch (FileNotExistsException e) {
					//Esto no debe darse ya que se comprueba al hacer el open.
					return null;
				} catch (NotSupportedExtensionException e) {
	...
	/*
	 * (non-Javadoc)
	 * @see org.gvsig.raster.dataset.IRasterDataSource#needEnhanced()
	 */
	public boolean needEnhanced() {
		//Returns true if at least one needs enhanced
		for(int i = 0; i < files.size(); i++){
			if(((RasterDataset)files.get(i)).needEnhanced())
				return true;
		}
		return false;
	}
