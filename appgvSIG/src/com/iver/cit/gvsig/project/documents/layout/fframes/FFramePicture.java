...
public class FFramePicture extends FFrame {
    ...
    private void drawSVG(Graphics2D g, Rectangle2D rect, Rectangle2D rv) {
    	if (rv!=null && !rv.intersects(rect))
    		return;
    	if (gvtRoot != null) {
    		 //Si no tiene atributo "viewBox" el método ViewBox.getViewTransform devuelve una transformación que no hace nada
    		if (elt.hasAttribute("viewBox")) {
    			...
    			double scaleX = 1;
    			double scaleY = 1;
    			//FIXME: Se podría mejorar el dibujado si parsearamos el atributo "preserveAspectRatio"
    			// según http://www.w3.org/TR/SVG/coords.html#PreserveAspectRatioAttribute
